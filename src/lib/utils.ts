...
/** ID produit catalogue sans suffixe panier (`-timestamp`). */
export function normalizeOrderProductBaseId(productId?: string): string {
  return (productId ?? '').replace(/-\d{13,}$/, '')
}
...
export function formatOrderItemName(item: OrderItemLikeForDisplay): string {
  ...
  if (baseId === BOX_DECOUVERTE_TROMPE_PRODUCT_ID && item.trompeDiscoverySelection?.length) {
    const labels = item.trompeDiscoverySelection.map(
...
export function expandOrderItemForProductionAggregate(item: OrderItemLikeForDisplay & { quantity: number }): Array<{
  ...
  if (base === BOX_DECOUVERTE_TROMPE_PRODUCT_ID && item.trompeDiscoverySelection?.length) {
