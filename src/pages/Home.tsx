...
import { ExperienceSection, SocialGridSection } from '../components/home/ExperienceSection'
...
import { AccessibilityControls } from '../components/AccessibilityProvider'
...
import { CATALOG_FILTERS, COMING_SOON_FEATURED_IDS, SIGNATURE_PRODUCT_IDS, type CatalogFilter } from '../lib/categories'
import { isProductOrderable } from '../lib/productHelpers'
...
export default function Home() {
  ...
  const { catalogProducts, availableProducts, comingSoonProducts } = useProducts()
  ...
  const [activeCategory, setActiveCategory] = useState<CatalogFilter>('Tout')
  ...
  const categories = CATALOG_FILTERS

  const signatureProducts = useMemo(
    () => catalogProducts.filter((p) => (SIGNATURE_PRODUCT_IDS as readonly string[]).includes(p.id)),
    [catalogProducts],
  )

  const featuredComingSoon = useMemo(
    () => {
      const ids = new Set(COMING_SOON_FEATURED_IDS)
      const featured = catalogProducts.filter((p) => ids.has(p.id as typeof COMING_SOON_FEATURED_IDS[number]))
      const rest = comingSoonProducts.filter((p) => !ids.has(p.id as typeof COMING_SOON_FEATURED_IDS[number]))
      return [...featured, ...rest].slice(0, 12)
    },
    [catalogProducts, comingSoonProducts],
  )

  // Si la catégorie active n'existe plus, revenir à Tout
  useEffect(() => {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Les pâtisseries': return <Eye size={20} />
      case 'Nos jus frais': return <Cup size={20} />
      case 'Canette Cake': return <Cup size={20} />
      case 'Cookies gourmands': return <Cookie size={20} />
      case 'Le salé': return <Package size={20} />
      case 'Fruits frais': return <Sparkles size={20} />
      case 'Chocolaterie': return <Cake size={20} />
      case 'Boxes': return <Package size={20} />
      case 'Tout': return <LayoutGrid size={20} />
  ...
  }, [activeCategory, searchQuery, catalogProducts, decorateMiniBoxProduct])
  ...
  const orderedProducts = useMemo(() => {
    ...
    if (activeCategory === 'Les pâtisseries') {
  ...
  const handleAddToCart = (product: Product) => {
    ...
    if (full && !isProductOrderable(full)) {
      showToast('Bientôt disponible chez Maison Mayssa.', 'info', 3500)
      return
    }
  ...
  useEffect(() => {
    ...
    import('../lib/firebase').then(({ getOrderRelease }) => {
      getOrderRelease(customer.phone).then((releasedAt: number | null) => {
    ...
    try {
      ...
      const itemsToDecrement = cart.flatMap((item) => {
        ...
        return getDecrItems(getOriginalProductId(item.product.id), item.quantity, PRODUCTS, {
          trompeDiscoverySelection: item.trompeDiscoverySelection,
        }).map((d: { productId: string; quantity: number }) => ({ ...d, label: item.product.name }))
      })
  ...
  return (
    <div className="min-h-screen bg-mayssa-soft selection:bg-mayssa-caramel/30 font-sans overflow-x-hidden">
      ...
      <div className="relative w-full px-4 pb-8 sm:pb-10 md:pb-12 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-x-hidden">

        <main id="main-content" className="mt-12 sm:mt-16 flex flex-col gap-16 sm:gap-24 md:gap-32 items-center" aria-label="Contenu principal">
          <section id="trompe-loeil-annecy" className="sr-only">
            <h2>Trompe l&apos;œil à Annecy</h2>
            <p>Découvrez nos trompes l&apos;œil pâtissiers à Annecy : créations artisanales Maison Mayssa. Précommande et retrait sur Annecy.</p>
          </section>

          <CategoryShowcaseGrid onCategorySelect={setActiveCategory} />
          ...
          <SignatureCreations
            ...
            getStock={getCardStock}
            isPreorderDay={isPreorderDay}
            dayNames={dayNames}
          />

          <ComingSoonSection
            products={featuredComingSoon}
            onViewDetail={setSelectedProductForDetail}
          />
          ...
          <motion.section
            ...
            className="space-y-8 w-full scroll-mt-28"
          >
            <div className="text-center space-y-3 mb-4">
              <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-mayssa-brown/45">La carte</p>
              <h2 className="font-display text-2xl sm:text-3xl md:text-4xl text-mayssa-brown">Précommandez vos créations artisanales</h2>
            </div>
